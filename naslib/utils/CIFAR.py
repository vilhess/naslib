...
class CIFAR10Dataset(Dataset):
    def __init__(self, root='../Dataset/CIFAR10/cifar-10-batches-py', train=True, download=False, transform=None):
        if train:
            self.data = self.load_all_batches(root, 'data_batch')
        else:
            self.data = self.load_batch(root, 'test_batch')
        if not transform :
...
if __name__=="__main__":
    ...
    test_dataset = CIFAR10Dataset(root='../Dataset/CIFAR10/cifar-10-batches-py', train=True, download=False, transform=None)
