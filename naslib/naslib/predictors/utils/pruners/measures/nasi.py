...
def compute_grad1(model: nn.Module, loss_type: str = 'mean') -> None:
    ...
    for layer in model.modules():
        ...
        if (layer_type not in _supported_layers) or (not hasattr(layer, 'activations')) or (not hasattr(layer, 'backprops_list')):
            continue
...
def ntk_trace(net, inputs, targets, mode, loss_fn, split_data=1):
    ...
    compute_grad1(net, loss_type='sum')
    ...
    score = np.sqrt(eigenvalues.cpu().numpy().sum() / N)
    return score